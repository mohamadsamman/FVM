...
# inner fluxes and upwind cells            
    ...
    for i in range(Nint):
        ...
        fluxint[i] = Tint[i]*(f_psi(b[k2]) - f_psi(b[k1])) 
        if (fluxint[i]>0): 
            upwind[i] = k1 
        else: 
            upwind[i] = k2 
...
# inner fluxes
   ...
   #flux2 = Ki[1]*(1-cs[kup])*fluxint[i]
        ...
        dflux1_hk1   = - Ki[0]*cs[kup]*Tint[i]*psipk1
        dflux1_hk2   =   Ki[0]*cs[kup]*Tint[i]*psipk2
        dflux1_cskup =   Ki[0]*fluxint[i]
  
        dflux2_hk1   = - Ki[1]*(1-cs[kup])*Tint[i]*psipk1
        dflux2_hk2   =   Ki[1]*(1-cs[kup])*Tint[i]*psipk2
        dflux2_cskup = - Ki[1]*fluxint[i]  
  
  #R(k1,0) = R(k1,0) + flux1 
  #R(k2,0) = R(k2,0) - flux1 
       
        A[2*k1,2*k1]    = A[2*k1,2*k1]       +  dflux1_hk1      
        A[2*k1,2*k2]    = A[2*k1,2*k2]       +  dflux1_hk2      
        A[2*k1,2*kup+1] = A[2*k1,2*kup+1]    +  dflux1_cskup      
       
        A[2*k2,2*k1]    = A[2*k2,2*k1]        -  dflux1_hk1      
        A[2*k2,2*k2]    = A[2*k2,2*k2]       -  dflux1_hk2      
        A[2*k2,2*kup+1] = A[2*k2,2*kup+1]    -  dflux1_cskup          
       
  #R(k1,1) = R(k1,1) + flux2
  #R(k2,1) = R(k2,1) - flux2        
 
        A[2*k1+1,2*k1]    = A[2*k1+1,2*k1]     +  dflux2_hk1      
        A[2*k1+1,2*k2]    = A[2*k1+1,2*k2]     +  dflux2_hk2      
        A[2*k1+1,2*kup+1] = A[2*k1+1,2*kup+1]  +  dflux2_cskup   

        A[2*k2+1,2*k1]   = A[2*k2+1,2*k1]      -  dflux2_hk1      
        A[2*k2+1,2*k2]    = A[2*k2+1,2*k2]     -  dflux2_hk2      
        A[2*k2+1,2*kup+1] = A[2*k2+1,2*kup+1]  -  dflux2_cskup           
...
for n in range(ndt):  # time loop 
    ...
    while ((normR/normR0>eps)and(itn<Newtmax)): #Newton loop 
        ...
        #dcsmax = np.max(np.abs(dcs))
        #theta = np.min([1,0.05/dcsmax])
        theta = 1
        #print(theta,normR/normR0)
        for k in range(N): # Newton update   
            h[k]  = h[k]  + dh[k]  
            cs[k] = cs[k] + dcs[k]*theta
